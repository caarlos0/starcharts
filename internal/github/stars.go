...
func (gh *GitHub) getFirstPageAndLastPage(ctx context.Context, repo Repository) ([]Stargazer, int, error) {
	...
	if resp.StatusCode == http.StatusNotModified {
		...
		// Get stars from cache
		if err := gh.cache.Get(key, &stars); err != nil {
			log.WithError(err).Warnf("failed to get %s from cache", key)
			// If we can't get from cache, we need to fetch again (ignoring etag this time)
			// Recursive call with cleared etag in cache?
			// Or just simple retry without etag.
			// Ideally we just delete etag and recurse.
			if err := gh.cache.Delete(etagKey); err != nil {
				log.WithError(err).Warnf("failed to delete %s from cache", etagKey)
			}
			return gh.getFirstPageAndLastPage(ctx, repo)
		}
	...
	// Even on 304, we expect Link header to be present if it's GitHub API.
...
func (gh *GitHub) getAllStargazersWithFirstPage(ctx context.Context, repo Repository, firstPageStars []Stargazer, lastPage int) (stars []Stargazer, err error) {
	...
	for page := 2; page <= lastPage; page++ {
		...
		wg.Go(func() error {
			...
			lock.Lock()
			defer lock.Unlock()
			// For small repos, we just append content. Sorting will happen later if needed,
			// or we can rely on the fact that we fetch everything.
			// However, typical concurrent fetching needs sorting or appending.
			// The original code was appending and then sorting.
			// But wait, the original code returned a sorted list.
